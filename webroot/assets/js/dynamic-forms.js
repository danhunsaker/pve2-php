    ...
    return {
      ...
      link: function ($scope, element, attrs) {
        ...
        var newElement = null,
          ...
          iterElem = element,
          model = null,
          unWatchMe = angular.noop;
        
        function updateMyTemplate(template) {
          if (angular.isObject(template)) {
            angular.forEach(element.contents(), function (elm) {
              elm = angular.element(elm);
              if (elm.attr('data-dynamic-content')) {
                elm.remove();
              }
            });
            angular.forEach(template, function (field, id) {
              if (!angular.isDefined(supported[field.type]) || supported[field.type] === false) {
                ...
                newElement = angular.element('<span data-dynamic-content="dynform"></span>');
                ...
                if (angular.isDefined(field.label)) {
                  ...
                  if (["image", "hidden"].indexOf(field.type) > -1) {
                    newElement.attr('data-dynamic-content', 'dynform');
                  }
                  ...
                  //  Everything else should be wrapped in a label tag.
                    ...
                    newElement = newElement.wrap('<label data-dynamic-content="dynform"></label>').parent();
            ...
            //  Refocus on the first autofocus element
            angular.forEach(element.find('*'), function(elm) {
              if (angular.element(elm).attr('autofocus')) {
                elm.focus();
                return false;
              } 
            });
          ...
          if (attrs.template) {
            unWatchMe = $scope.$watch(attrs.template, function(template, prev) {
              if (angular.equals(template, prev)) {
                return;
              }
              unWatchMe();
              updateMyTemplate(template);
            });
          }
        };
